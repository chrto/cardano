...
-- cabal test --test-options=" redeemerUntyped-test
-- or
-- cabal test --test-options="--quickcheck-tests 10000" redeemerUntyped-test
...
import           Plutus.V2.Ledger.Api (BuiltinData, Value, ToData(toBuiltinData)
                                     , PubKeyHash, TxOutRef, TxOut(txOutValue)
                                     , adaSymbol, adaToken)
...
import           Test.QuickCheck (Testable(property), Property, (==>), collect, Arbitrary(arbitrary), Gen, choose)
  ...
  $ do
    testGroup
      ...
      [ testGroup
          ...
          [ testGroup
              "HappyPath"
              [ happyPath "User 1 locks and user 2 takes with R = 42." $ runUnitTest 42 $ adaValue 150]
          , testGroup
              "ErrorPath"
              [ errorPath "User 1 locks and user 2 do not takes with R != 41." $ runUnitTest 41 $ adaValue 200]
          ]
      , testGroup
          "PropertyTest"
          [ testGroup
  ...
  where
    ...
    spendValidatorUtxos = scriptRefUTxOs <&> fst <&> spendValidatorUtxo
      where
        ...
        spendValidatorUtxo txOutRef =
          let buildRedeemer = toBuiltinData redeemer
...
testScript :: Bool -> Integer -> Value -> Run Bool
testScript shouldPass redeemer value = do
  ...
  let expectedPkh_1 = adaValue $ (-)
        ...
        (valueOf value adaSymbol adaToken)
      expectedPkh_2 = if shouldPass
                      then initialBalance <> value
                      else initialBalance
...
---------------------------------------------------------------------------------------------------
------------------------------------ UNIT TEST FUNCIONS--------------------------------------------
runUnitTest :: Integer -> Value -> Run ()
runUnitTest redeemer value = testScript True redeemer value
...
runChecks shouldPass redeemer value = monadic property check
  where
    check :: PropertyM Run ()
    check = run (testScript shouldPass redeemer value)
      >>= assert
