...
-- 'cabal test --test-options=" vesting-parametrized-two-test'
-- or
-- 'cabal test --test-options="--quickcheck-tests 10000" vesting-parametrized-two-test' to run 10000 test
-- or
-- 'cabal test --test-options="--quickcheck-tests 10 --use-collect-mode True" vesting-parametrized-two-test' to print generated values
...
import qualified VestingParametrizedTwo as OnChain
...
---------------------------------------------------------------------------------------------------
------------------------------------------- DATA --------------------------------------------------
validatorScript :: PubKeyHash -> POSIXTime -> TypedValidator () ()
validatorScript beneficiary = TypedValidator . toV2 . OnChain.validator beneficiary
  ...
  where
    ...
    txOutput = payToScript buildValidator buildDatum value
      where
        ...
        buildValidator = validatorScript beneficiary deadline
  ...
  where
    ...
    spendValidatorUtxos = scriptRefUTxOs <&> fst <&> spendValidatorUtxo
      where
        ...
        validator = validatorScript beneficiary deadline
...
testScript shouldPass cnt beneficiary deadline sender claimer value = do
  ...
  (currentTimeRad 100 <&> (,))            -- create tx valid range
    <*> (utxoAt (validatorScript beneficiary deadline)          -- get UTxOs sitting at script address
