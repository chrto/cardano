...
GIFT_VALUE=3000000
OUT_NAME=redeemerCustomInteger
ERA=latest
NAME=""
TXIN=""
...
function show_help() {
    ...
    echo "  -v, --value <amount of lovelace>    Set the value in lovelace, to send to gift contract. Defoult is $GIFT_VALUE"
...
cardano-cli address build \
    --payment-script-file "$ASSETS_DIR/$OUT_NAME.plutus" \
    --testnet-magic 2 \
    --out-file "$ASSETS_DIR/$OUT_NAME.addr"

# Build the transaction
cardano-cli $ERA transaction build \
    --testnet-magic 2 \
    --tx-in "$TXIN" \
    --tx-out "$(cat "$ASSETS_DIR/$OUT_NAME.addr") + $GIFT_VALUE lovelace" \
    --tx-out-inline-datum-file "$ASSETS_DIR/unit.json" \
    --change-address "$(cat "$KEYS_PATH/$NAME.addr")" \
    --out-file "$ASSETS_DIR/$OUT_NAME.txbody"

# Sign the transaction
cardano-cli $ERA transaction sign \
    --tx-body-file "$ASSETS_DIR/$OUT_NAME.txbody" \
    --signing-key-file "$KEYS_PATH/$NAME.skey" \
    --testnet-magic 2 \
    --out-file "$ASSETS_DIR/$OUT_NAME.tx"

# Submit the transaction
cardano-cli $ERA transaction submit \
    --testnet-magic 2 \
    --tx-file "$ASSETS_DIR/$OUT_NAME.tx"

tid=$(cardano-cli $ERA transaction txid --tx-file "$ASSETS_DIR/$OUT_NAME.tx")
