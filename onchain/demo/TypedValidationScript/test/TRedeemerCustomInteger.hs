...
-- 'cabal test --test-options=" redeemerCustomInteger-test'
-- or
-- 'cabal test --test-options="--quickcheck-tests 10000" redeemerCustomInteger-test' to run 10000 test
-- or
-- 'cabal test --test-options="--quickcheck-tests 1000 --use-collect-mode True" redeemerCustomInteger-test' to print generated values
...
import qualified RedeemerCustomInteger as OnChain
  ...
  where
    ...
    spendValidatorUtxos = scriptRefUTxOs <&> fst <&> spendValidatorUtxo
      where
        ...
        spendValidatorUtxo txOutRef = spendScript validatorScript txOutRef buildRedeemer ()

        buildRedeemer :: OnChain.MyRedeemer
        buildRedeemer = OnChain.MkMyRedeemer redeemer
