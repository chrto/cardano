...
-- 'cabal test --test-options=" vesting-parametrized-test'
-- or
-- 'cabal test --test-options="--quickcheck-tests 10000" vesting-parametrized-test' to run 10000 test
-- or
-- 'cabal test --test-options="--quickcheck-tests 10 --use-collect-mode True" vesting-parametrized-test' to print generated values
...
import qualified  VestingParametrized as OnChain
    ...
    tests = askOption $ \(UseCollectMode collectMode) ->
      testGroup
        ...
        [ testGroup
            ...
            [ testGroup
                "HappyPath"
                [ happyPath "Sender locks 100 Ada into Vesting contract and beneficiary should take them after deadline." $ initUsers 2 >>= (\[sender,claimer] -> runUnitTest 1 (OnChain.VestingParams claimer 500) sender claimer 100)
                , happyPath "Sender locks 3x100 Ada into Vesting contract and beneficiary should take them all after deadline." $ initUsers 2 >>= (\[sender,claimer] -> runUnitTest 3 (OnChain.VestingParams claimer 500) sender claimer 100)
                ]
            , testGroup
                "ErrorPath"
                [ errorPath "Sender locks 100 Ada into Vesting contract and beneficiary should not take them before deadline." $ initUsers 2 >>= (\[sender,claimer] -> runUnitTest 1 (OnChain.VestingParams claimer 5000) sender claimer 100)
                , errorPath "Sender locks 100 Ada into Vesting contract and claimer should not take them, if not beneficiary." $ initUsers 3 >>= (\[sender,claimer, beneficiary] -> runUnitTest 1 (OnChain.VestingParams beneficiary 500) sender claimer 100)
                ]
            ]
        , testGroup
            "PropertyTest"
...
---------------------------------------------------------------------------------------------------
------------------------------------------- DATA --------------------------------------------------
validatorScript :: OnChain.VestingParams -> TypedValidator () ()
...
---------------------------------------------------------------------------------------------------
-------------------------------------- HELPER FUNCIONS---------------------------------------------
lockingTransaction :: OnChain.VestingParams -> UserSpend -> Value -> Tx
lockingTransaction params utxosWithChange value = spendUtxos <> txOutput
  where
    ...
    txOutput = payToScript buildValidator buildDatum value
      where
        ...
        buildValidator :: TypedValidator () ()
        buildValidator = validatorScript params


consumingTransaction :: OnChain.VestingParams -> PubKeyHash -> [(TxOutRef, TxOut)] -> Tx
consumingTransaction params claimer scriptRefUTxOs =
  ...
  where
    ...
    spendValidatorUtxos = scriptRefUTxOs <&> fst <&> spendValidatorUtxo
      where
        ...
        spendValidatorUtxo txOutRef = spendScript validator txOutRef () ()

---------------------------------------------------------------------------------------------------
--------------------------------------- TEST FUNCIONS----------------------------------------------
-- Function to test if both creating and consuming script UTxOs works properly
testScript :: Bool -> Integer -> OnChain.VestingParams -> PubKeyHash -> PubKeyHash -> Value -> Run Bool
testScript shouldPass cnt params sender claimer value = do

  replicateM_ (fromIntegral cnt)
    $ spend sender value                          -- get inputs and change output for locking tx
    <&> (\sp -> lockingTransaction params sp value)     -- build locking transaction
    >>= submitTx sender                           -- submit transaction

  waitUntil consumeTxAfter            -- wait 1000 ms

  (currentTimeRad 100 <&> (,))            -- create tx valid range
    <*> (utxoAt (validatorScript params)          -- get UTxOs sitting at script address
    <&> consumingTransaction params claimer)      -- build unlocking transaction
...
---------------------------------------------------------------------------------------------------
------------------------------------ UNIT TEST FUNCIONS--------------------------------------------
runUnitTest :: Integer -> OnChain.VestingParams -> PubKeyHash -> PubKeyHash -> Integer -> Run ()
runUnitTest cnt params sender claimer value = testScript True cnt params sender claimer (adaValue value)
...
runChecks shouldPass deadline = monadic property check
  where
    check :: PropertyM Run ()
    check = run (initUsers 2 >>= (\[sender, claimer] -> testScript shouldPass 1 (OnChain.VestingParams claimer deadline) sender claimer (adaValue 100)))
      >>= assert
