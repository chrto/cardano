...
function show_help() {
    ...
    echo "  -v, --value <amount of lovelace>    Set the value in lovelace, to send to gift contract. Defoult is $VEST_VALUE"
    echo "  -o, --output <output name>          Set the name of output file name."
...
if [ -z "$OUTPUT_NAME" ]; then
    >&2 echo "expected contract/output file name as argument"
    show_help
    exit 1
fi
...
cardano-cli $ERA transaction build \
    --testnet-magic 2 \
    --tx-in "$TXIN" \
    --tx-out "$(cat "$ASSETS_DIR/$OUTPUT_NAME.addr") + $VEST_VALUE lovelace" \
