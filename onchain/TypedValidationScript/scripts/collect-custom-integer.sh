...
OUTPUT_NAME=redeemerCustomInteger
ERA=latest
NAME=""
TXIN=""
COLLATERAL=""
...
if [ -z "$NAME" ]; then
    >&2 echo "expected keys owner name as argument - who will receive assets!"
    show_help
    exit 1
fi
...
if [ -z "$TXIN" ]; then
    >&2 echo "expected input transaction id as argument - burn contract UTXO!"
    show_help
    exit 1
fi
...
cardano-cli $ERA transaction build \
    ...
    --tx-in-redeemer-file "$ASSETS_DIR/redeemerCustomInteger.json" \
...
cardano-cli $ERA transaction sign \
    --tx-body-file "$ASSETS_DIR/collect-$OUTPUT_NAME.txbody" \
    --signing-key-file "$KEYS_PATH/$NAME.skey" \
    --testnet-magic 2 \
    --out-file "$ASSETS_DIR/collect-$OUTPUT_NAME.tx"

# Submit the transaction
cardano-cli $ERA transaction submit \
    --testnet-magic 2 \
    --tx-file "$ASSETS_DIR/collect-$OUTPUT_NAME.tx"

tid=$(cardano-cli $ERA transaction txid --tx-file "$ASSETS_DIR/collect-$OUTPUT_NAME.tx")
