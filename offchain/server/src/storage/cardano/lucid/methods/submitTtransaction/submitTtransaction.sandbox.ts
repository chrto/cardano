import submitTtransaction from './submitTtransaction';
import buildTtransactionFromAddress from '../buildTtransactionFromAddress/buildTtransactionFromAddress';
import signTtransaction from '../signTtransaction/signTtransaction';
...
Promise.resolve(Either.right<AppError, AppConfig>({} as AppConfig))
  ...
  .then(asyncBind((lucid: LucidStorage) =>
    isMissing(process.env.CARDANO_TX_SIGNED)
      ? buildTtransactionFromAddress(lucid.lucidEvolution)(process.env.CARDANO_WALLET_ADDRESS, process.env.CARDANO_GIFT_ADDRESS, Data.to(new Constr(0, [])), BigInt('10000000'))
        ...
        .then(tap(sniff))
        .then(asyncBind(cbor => submitTtransaction(lucid.lucidEvolution)(cbor, process.env.CARDANO_PRIVATE_KEY)))
      : submitTtransaction(lucid.lucidEvolution)(process.env.CARDANO_TX_SIGNED, process.env.CARDANO_PRIVATE_KEY)
  ))
  .then(doer({
    right: sniff,
