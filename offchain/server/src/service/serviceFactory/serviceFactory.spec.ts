...
describe(`service`, () => {
  describe(`pluginSdkService module`, () => {
    ...
    let scriptServiceFactory: Fcn<[], ScriptService> = () => ({} as ScriptService);
    let scriptServiceReferenceFactory: Fcn<[], ScriptReferenceService> = () => ({} as ScriptReferenceService);
    ...
    beforeAll(() => {
      pluginSdkService = pluginSdkServiceUnbound
        (authenticationServiceFactory, userServiceFactory, scriptServiceFactory, scriptServiceReferenceFactory, cardanoServiceFactory, cardanoKupoServiceFactory)
        ({} as AppConfig)
    ...
    it(`Should return 'PluginSdkService'`, () => {
      ...
      expect(Object.keys(pluginSdkService).length).toEqual(8);
      ...
      expect(pluginSdkService).toHaveProperty('scriptService');
      expect(pluginSdkService).toHaveProperty('scriptReferenceService');
