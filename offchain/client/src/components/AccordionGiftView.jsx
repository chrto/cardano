...
export default function AccordionGiftView({ scriptUtxos, scriptAddress, selectedScript } ) {
  ...
  return (
    <form className="accordion-form">
      ...
      <AccordionItem title={"Script Utxo's with Unit InLine Datum (" + scriptUtxos.filter(filterInlineDatumUnit).length + ")"} isOpen={openIndex === 0} onToggle={() => toggle(0)}>
        <Table
          headers={['UTXO', 'Value [Lovelace]', 'Value [Ada]']}
          values={scriptUtxos.filter(filterInlineDatumUnit).map(convertUtxos).sort((x, y) => y[2] - x[2])}
        />
      </AccordionItem>
